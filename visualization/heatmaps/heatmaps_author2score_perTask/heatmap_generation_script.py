...
# Function to calculate the median score for all the models for a given author on a given task
def calculate_median_score_for_author(author, task_index, score_label):
    # Filter scores by author and task index
    filtered_scores = {k: v for k, v in scores_dict.items() if
                       any(score['author'] == author for score in v) and gpt4_dict[k]["task_index"] == task_index}

    print(filtered_scores)
    # Extract the scores for all the models
...
def calculate_median_score_for_model(model, task_index, score_label):
    ...
    # Extract the scores for all the authors
    score = [score[score_label] for scores in filtered_scores.values() for score in scores]
...
def generate_dataframe_for_heatmap(task_index):
    ...
    df = pd.DataFrame(0, index=score_labels, columns=author_labels)
    
    # Populate the DataFrame with median scores for each score-model combination
    for score_label in score_labels:
        for author in author_labels:
            df.at[score_label, author] = calculate_median_score_for_author(author, task_index, score_label)
